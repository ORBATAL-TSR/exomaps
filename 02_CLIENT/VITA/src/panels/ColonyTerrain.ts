...
export interface TerrainParams {
  ...
  /** Vertex displacement scale — MUST match uDisplacement in ProceduralWorld shader.
   *  Formula: base(0.055) + mountainHeight*0.22 + volcanism*0.14.
   *  Defaults to 0.055 if not provided (flat terrain). */
  displacement?: number;
}
...
function noise3(x: number, y: number, z: number): number {
  ...
  // Quintic smoothstep — MUST match vertex shader exactly (vert.ts uses same formula)
  // Old cubic (3-2t) caused height mismatch, making objects float/sink on terrain
 ...
 *  Uses tp.displacement when available (preferred — computed to match shader exactly).
 *  Falls back to the explicit displacement param if tp.displacement is not set. */
export function toTerrainSpherePos(
  ...
  // Prefer tp.displacement (matches shader uDisplacement exactly)
  const disp = tp.displacement ?? displacement;
  const r = 1 + (terrain - 0.5) * disp + offset;
